...
Our favorite friend, double differential scattering, can be simplified. When we can't simplify it, we at least need to represent it in a way we can numerically manage. Let's start by recalling
...
If \textbf{scattering is not isotropic}, what do we do? There are several approaches, but this one makes the most sense to me (different from Duderstadt and Hamilton).
...
as follows
\[\Sigma_s(\vOmega' \cdot \vOmega) = \sum_{l=0}^{\infty} \frac{2l+1}{4\pi} \Sigma_{sl} P_l(\vOmega')P_l(\vOmega) = \sum_{l=0}^{\infty} \frac{2l+1}{4\pi} \Sigma_{sl} P_l(\mu_0)\:. \]
%
We get the scattering expansion coefficients from the orthogonality of Legendre Polynomials:
\[
\Sigma_{sl} = 2\pi \int_{-1}^1 d\mu_0 \: P_l(\mu_0)\Sigma_s(\mu_0) \:.
\]
%
